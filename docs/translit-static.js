...
//   <span class="sa-cap">देवनागरी</span>          same, but Title Cased in IAST
...
const SA_CAP_SELECTOR = ".sa-cap";
const SA_TITLE_SELECTOR = "[data-sa-title]";

// .sa-cap title-cases each whitespace-separated word, but only under IAST.
// Devanagari is caseless, and the other romanizations encode case
// significantly -- ITRANS/HK/SLP1 all use capitals as distinct letters, so
// capitalizing "kAdambarI" would silently rewrite which phonemes it spells.
...
export function renderStaticTranslit(scheme, root = document) {
  ...
  for (const el of root.querySelectorAll(SA_CAP_SELECTOR)) {
    if (el.dataset.deva === undefined) el.dataset.deva = el.textContent;
    const out = translit(el.dataset.deva, scheme);
    el.textContent = scheme === "iast" ? capitalizeWords(out) : out;
  }
